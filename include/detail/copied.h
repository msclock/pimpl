...
namespace pimpl
{
	...
	struct impl_ptr_policy::copied
	{
		...
		{
			if (typename alloc_traits::propagate_on_container_move_assignment() || o.get_allocator() == get_allocator())
			{
			...
			else
			{
				...
				/**/ if (impl_ == o.impl_)
					;
				else if (impl_ && o.impl_)
					traits_type::assign(impl_.get(), std::move(*o.impl_));
				else if (impl_ && !o.impl_)
					impl_.reset();
				else if (!impl_ && o.impl_)
					impl_ = traits_type::make(get_allocator(), std::move(*o.impl_));
				o.impl_.reset();
			}
			return *this;
		}
		this_type& operator=(this_type const& o)
		{
			...
			/**/ if (impl_ == o.impl_)
				...
				traits_type::assign(impl_.get(), *o.impl_);
			else if (impl_ && !o.impl_)
				impl_.reset();
			else if (!impl_ && o.impl_)
				impl_ = traits_type::make(get_allocator(), *o.impl_);
