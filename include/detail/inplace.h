...
namespace pimpl
{
	...
	namespace impl_ptr_policy
	{
		...
		template <size_t s, size_t a = std::size_t(-1)>
		struct storage
		{
	...
	namespace detail
	{
		...
		template <typename T, typename inner_alloc>
		...
		private:
			...
			using use_alloc = typename std::conditional<
				std::uses_allocator<TT, Alloc>::value && std::is_constructible<TT, std::allocator_arg_t, Alloc, Args...>::value, construct_with_alloc_tag, typename std::conditional<std::uses_allocator<TT, Alloc>::value && std::is_constructible<TT, Args..., Alloc>::value, construct_with_alloc_last, construct_without_alloc>::type>::type;
			...
			void construct_with_inner_allocator(construct_with_alloc_last, derived_type* p, arg_types&&... args)
			{
				::new (static_cast<void*>(p)) derived_type(
					std::forward<arg_types>(args)..., *inner_allocator);
			}
			...
			{
				::new (static_cast<void*>(p)) derived_type(
					std::allocator_arg, *inner_allocator, std::forward<arg_types>(args)...);
			}
	...
	struct detail::basic_inplace // Proof of concept
	{
		...
		using storage_type = pimpl::aligned_storage<size_type::size, size_type::alignment>;
		using traits_type = detail::traits::copyable<impl_type, inplace_allocator<impl_type, allocator>>;
		...
		constexpr basic_inplace(std::nullptr_t, const allocator_type&)
		{
			static_assert(exists_type(false) == false, "Constructing null-state is prohibited.");
		}
		constexpr basic_inplace(this_type const& o)
		{
			_assign(o);
		}
		constexpr basic_inplace(this_type&& o)
		{
			_assign(std::move(o));
		}
		constexpr this_type& operator=(this_type const& o)
		{
			return _assign(o);
		}
		constexpr this_type& operator=(this_type&& o)
		{
	...
	private:
		...
		void _construct(alloc_arg&& a0, arg_types&&... args)
		{
			...
			traits_type::emplace(a, reinterpret_cast<derived_type*>(storage().address()), std::forward<arg_types>(args)...);
			set_exists(true);
		}
			...
			_assign(T&& o)
		{
			...
			else if (exists && o_exists)
				traits_type::assign(get(), std::forward<uref>(*o.get()));
			else if (exists && !o_exists)
			{
				set_exists(false);
				traits_type::destroy(a, get());
			...
			{
				traits_type::construct(a, storage().address(), std::forward<uref>(*o.get()));
				set_exists(true);
			}
		...
		constexpr storage_type& storage()
		{
