...
namespace pimpl
{
	namespace detail
	{
		...
		struct dealloc_guard
		{
			...
			constexpr value_type* get() noexcept { return pimpl::to_address(ptr_); }
	...
	struct detail::traits::base
	{
		...
		struct deleter : private alloc_type
		{
			...
			constexpr allocator_type& get_allocator() { return *this; }
		};
		...
		static void emplace(
			typename alloc_traits::
				template rebind_alloc<
					typename std::remove_cv<derived_type>::type>& alloc,
			derived_type* p, arg_types&&... args)
		{
	...
	protected:
		...
		{
			detail::dealloc_guard<alloc_type> ap(a, std::move(p));
	...
	struct detail::traits::unique final : base<unique<impl_type, allocator>, impl_type, allocator>
	{
		...
		void do_assign(pointer, impl_type const&) const override { PIMPL_ASSERT(!"not implemented"); }
		void do_assign(pointer, impl_type&&) const override { PIMPL_ASSERT(!"not implemented"); }
		void do_construct(alloc_type&, void*, impl_type const&) const override { PIMPL_ASSERT(!"not implemented"); }
		void do_construct(alloc_type&, void*, impl_type&&) const override { PIMPL_ASSERT(!"not implemented"); }
		ptr_type do_make(alloc_type& a, impl_type const&) const override
		{
			PIMPL_ASSERT(!"not implemented");
			return ptr_type(nullptr, a);
		}
		ptr_type do_make(alloc_type& a, impl_type&&) const override
		{
			PIMPL_ASSERT(!"not implemented");
			return ptr_type(nullptr, a);
		}
