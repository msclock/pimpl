...
namespace pimpl
{
	...
	struct impl_ptr
	{
		...
		template <typename derived_impl_type, typename allocator, typename... arg_types, typename = typename std::enable_if<std::uses_allocator<policy_type, allocator>::value>::type>
		void
		...
		template <typename allocator, typename... arg_types, typename = typename std::enable_if<std::uses_allocator<policy_type, allocator>::value>::type>
		void
		emplace(std::allocator_arg_t, allocator&& a, arg_types&&... args)
		{
		...
		{
			PIMPL_ASSERT(impl_.get());
			return reinterpret_cast<ImplT*>(impl_.get());
		}

	protected:
		template <typename, template <typename, typename...> class, typename...>
		friend struct impl_ptr;
	...
	struct is_impl_ptr : std::false_type
	{
	};

	template <typename T>
	struct is_impl_ptr<T, void_type<typename T::impl_ptr_type>> : std::true_type
	{
	};
} // namespace pimpl
