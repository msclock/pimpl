...
template <>
struct pimpl::impl_ptr<AlwaysInPlace>::implementation
{
...
static_assert(sizeof(AlwaysInPlace) == sizeof(pimpl::impl_ptr<AlwaysInPlace>::implementation),
			  "No memory size overhead for always_inplace is permitted");
static_assert(alignof(AlwaysInPlace) == alignof(pimpl::impl_ptr<AlwaysInPlace>::implementation),
...
bool AlwaysInPlace::operator==(AlwaysInPlace const& that) const
{
